...
var AI = client.connect("https://game-ten-gamma.vercel.app");
...
function calcDamage(source, target, type) {
  ...
  switch (type) {
    ...
    case "2":
      ...
      if (
        userPokemons[source].supermoves.energyCost * 100 >
        userPokemons[source].mp
      ) {
        res = 0;
...
io.on("connection", function (socket) {
      ...
      if (userStatus[socket.id].status == "home") {
        ...
        if (room.status == "waiting") {
          ...
          if (room.players[0] == AI.id) {
            ...
            userStatus[room.ownner] = Object.assign(userStatus[room.ownner], {
              status: "home",
            }); //change home ownner's userStatus
          }
        }
      } else if (userStatus[socket.id].status == "guest") {
        //user is room guest
        var room = vs[userStatus[socket.id].roomId];
  ...
  socket.on("enter room", function (data) {
    ...
    var notice =
      "<p>Welcome to pokemon fight game!</p><p>Enter <b>#</b> to select a pokemon.</p><p>Enter <b>?</b> to view the battle instructions.</p><p>Enter <b>~</b> to view your pokemon infomation at any time.</p><p>Enter <b>+</b> to add AI.</p><p>Enter <b>-</b> to reset game status.</p><p>Have fun!</p>";
  ...
  socket.on("confirm", function () {
    ...
    if (userStatus[socket.id] == undefined) {
      ...
      for (var i = 0; i < room.players.length; i++) {
        ...
        var msg =
          "Please enter your battle commands ...(enter ? to see instructions)";
        io.to(player).emit("notice", msg);
      }
    }
  ...
  socket.on("chat message", function (msg) {
    ...
    switch (op) {
      case "#": // pokemon info
        if (userStatus[socket.id] != undefined) {
          var msg =
            "You cannot change your pokemon at this moment. To reset game status, enter <b>-</b>";
          socket.emit("notice", msg);
          break;
        }
        ...
        var index =
          1 <= parseInt(post) && parseInt(post) <= 151
            ? post
        ...
        break;
      case "@":
        if (!userPokemons[socket.id]) {
          //undefined
          var msg =
            "System Message: You should input '#' first to choose a pokemon!";
          socket.emit("notice", msg);
        } else {
          //else##
          //handle rsp
          ...
          if (!isCommandValid(str)) {
            ...
            //big big program starts here!
            if (userStatus[socket.id] == undefined) {
              //user has not confirm(ready to battle)
              var error = "You must confirm a pokemon selection first!";
              socket.emit("notice", error);
            } else {
              //first, find user's room
              var room = vs[userStatus[socket.id].roomId];
              var roomid = room.id;
              ...
              if (room.isAI && userPokemons[AI.id] != undefined) {
                ...
                var AI_commands = "";
                if (
                  userPokemons[AI.id].mp >=
                  userPokemons[AI.id].supermoves.energyCost
                ) {
                  AI_Oricommands = "22";
                  AI_commands =
                    userPokemons[AI.id].supermove_command +
                    userPokemons[AI.id].supermove_command;
                } else {
                  ...
                  switch (rand1) {
                    case 0:
                      ...
                      AI_commands =
                        c1 +
                        userPokemons[AI.id].move_command +
                        c2 +
                        userPokemons[AI.id].move_command;
                      break;
                    case 1:
                ...
                function showSupermoveRes1() {
                  if (++k1 >= round_res.supermove1.length + 1) {
                    if (!battleOn(round_res.user1, round_res.user2)) {
                      ...
                      var notice =
                        "<span class = 'glyphicon glyphicon-queen gold'></span> Battle ends! The winner is <b>" +
                        userPokemons[winner].username +
                        "</b>";
      ...
      case "~":
        if (userPokemons[socket.id] != undefined) {
          ...
          var notice =
            "You have not selected a pokemon yet! To do that,enter #";
          socket.emit("notice", notice);
        }
        break;
      case "+":
        ...
        // AI join battle
        var AI_candidates = [
          3, 6, 9, 31, 34, 59, 62, 65, 68, 71, 76, 94, 103, 107, 110, 112, 123,
          127, 128, 130, 131, 141, 143, 144, 145, 146, 149, 150, 151,
        ];
      ...
      case "-":
          ...
          if (userStatus[socket.id].status == "home") {
            ...
            if (room.status == "waiting") {
              ...
              if (room.players[0] == AI.id) {
                ...
                room.status = "waiting";
                room.ownner = room.players[0]; //transfer ownnership to another user
                userStatus[room.ownner] = Object.assign(
                  userStatus[room.ownner],
                  { status: "home" }
                ); //change home ownner's userStatus
              }
            }
        ...
        var notice =
          "Your game status is reset. You can enter enter <b>#</b> to choose another pokemon.";
        socket.emit("notice", notice);
