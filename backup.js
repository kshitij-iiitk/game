...
socket.on("left room", function (msg) {
  $("#status").html("&nbsp;");
  $("#messages").append($('<li class = "">').text(msg));
  scrollToBottom();
});
...
socket.on("online", function (data) {
  let onlineUsers = `Online users (${data.nums}):<br>`;

  const dataList = Array.isArray(data.list) ? data.list : data.list.split(" ");

  for (let index = 0; index < data.nums; index++) {
    onlineUsers += dataList[index] + "<br>";
  }

  $("#online").html(onlineUsers);
});

socket.on("notice", function (err) {
  $("#status").html("&nbsp;");
  $("#messages").append($('<li class = "">').html(err));
  scrollToBottom();
});
  ...
  $("#messages").append(
    $("<li>").html(
      "<b>Supermove:</b> " +
        res.supermoves.name +
        " | <b>Damage:</b> " +
        res.supermoves.damage +
        " | <b>Power Cost:</b> " +
        res.supermoves.energyCost * 100 +
        " | <b>Command:</b> " +
        res.supermove_command
    )
  );
  $("#messages").append(
    $("<li>").html(
      "<button class = 'evolution' onclick = 'evolve()'>evolve</button><button class = 'confirm' onclick = 'confirm()'>confirm</button>"
    )
  );
  $("#messages").append(
    $('<li class = "green">').text("input # to continue switching pokemons.")
  );
  scrollToBottom();
});
...
socket.on("single_res", function (res) {
  ...
  var statusArr = [
    '<span class = "gray">Lost</span>',
    '<span class = "green">Draw</span>',
    '<span class = "orange">Win</span>',
  ];
  for (var key in res) {
    if (res[key].username == nickname) {
      self = res[key];
    ...
    $("#messages").append(
      $("<li>").html(
        "<b>" +
          ...
          "</b> releases <b>" +
          attacker.pokemon.moves.name +
          '</b>, causes <span class ="red"><b>' +
          attacker.damage +
          "</b></span> damage."
      )
    );
    ...
    $("#messages").append(
      $("<li>").html(
        "<b>" +
          attacker.pokemon.name +
          "</b> releases <b>" +
          attacker.pokemon.supermoves.name +
          '</b>, causes <span class ="red"><b>' +
          attacker.damage +
          '</b></span> damage. <span class = "green">' +
          attacker.message +
          "</span>"
      )
    );
